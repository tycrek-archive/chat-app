...
exports.init = () => {
	return new Promise((resolve, reject) => {
		fse.readJson(this.getPath('config.json'))
			.then((obj) => config = obj)
			.then(() => Psql.init())
			.then(() => resolve(this.config().server))
...
exports.generateToken = (length = this.config().security.tokenLength) => crypto.randomBytes(length).toString('hex');
...
exports.validate = (req) => {
	return new Promise((resolve, reject) => {
		...
		if (token == null || token.length == 0) return reject(this.config().response.unauthorized);

		Psql.sessionGet(token).then((dataset) => {
			if (dataset.length === 0) reject(this.config().response.unauthorized);
			else {
				let now = this.utcStamp();
				let expiry = this.tdFormat(dataset[0].expiry, 'x');
				if (expiry > now) resolve();
				else reject(this.config().response.forbidden);
			}
		}).catch((err) => {
			reject(this.buildResponse(500, '500', { err: err }));
		});
	});

	function _isPublicRoute(path) {
		let routes = this.config().publicRoutes;
